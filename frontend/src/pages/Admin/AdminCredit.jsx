...
import GiveCreditModal from "../../components/Admin/GiveCreditModal";
import addCredit from "../../hooks/admin/addCredit";
...
const AdminCredit = () => {
  ...
  const handleCredit = async (data) => {
    try {
      const { data: res } = await addCredit(data.userEmail, data.userId, {
        data,
      });
      if (res.success) {
        Swal.fire({
        ...
        setUsers((prev) => {
          const updatedUsers = prev.map((user) =>
            user._id === res.user._id ? res.user : user,
          );
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
                Credit <span className="text-red-600">Management</span>
              </h1>
              <p className="text-gray-600 mt-1 text-sm sm:text-base">
                Manage all registered users Credit
              </p>
            </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-gray-900">
                        {user.subscription.minute.toFixed(0, 2) || 0} mins
                      </span>
                    </div>
                  </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredUsers.map((user) => (
                    <tr
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {user.subscription?.minute?.toFixed(2) || 0}
                        </div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
